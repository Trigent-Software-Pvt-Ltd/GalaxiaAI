...
import { getClerkBaseUrl, getGalaxiaApiUrl, PRODUCTION_CLERK_BASE_URL } from "./config.js"
...
export class WebAuthService extends EventEmitter<AuthServiceEvents> implements AuthService {
	...
	public async login(landingPageSlug?: string, useProviderSignup: boolean = false): Promise<void> {
		try {
			...
			const publisher = packageJSON?.publisher ?? "TrigentSoftwareInc"
			const name = packageJSON?.name ?? "galaxia"
			...
			const url = landingPageSlug
				? `${getGalaxiaApiUrl()}/l/${landingPageSlug}?${params.toString()}`
				: useProviderSignup
					? `${getGalaxiaApiUrl()}/extension/provider-sign-up?${params.toString()}`
					: `${getGalaxiaApiUrl()}/extension/sign-in?${params.toString()}`

			await vscode.env.openExternal(vscode.Uri.parse(url))
		} catch (error) {
			const context = landingPageSlug ? ` (landing page: ${landingPageSlug})` : ""
			this.log(`[auth] Error initiating Galaxia Cloud auth${context}: ${error}`)
			throw new Error(`Failed to initiate Galaxia Cloud authentication${context}: ${error}`)
		}
	}

	/**
	 * Handle the callback from Galaxia Cloud
	 *
	 * This method is called when the user is redirected back to the extension
	 * after authenticating with Galaxia Cloud.
	 *
	...
	public async handleCallback(
		...
		if (!code || !state) {
			...
			if (vscode) {
				vscode.window.showInformationMessage("Invalid Galaxia Cloud sign in url")
			}
		...
		try {
			...
			if (vscode) {
				vscode.window.showInformationMessage("Successfully authenticated with Galaxia Cloud")
			}

			this.log("[auth] Successfully authenticated with Galaxia Cloud")
		} catch (error) {
			this.log(`[auth] Error handling Galaxia Cloud callback: ${error}`)
			this.changeState("logged-out")
			throw new Error(`Failed to handle Galaxia Cloud callback: ${error}`)
		}
	}
	...
	public async logout(): Promise<void> {
		...
		try {
			...
			if (vscode) {
				vscode.window.showInformationMessage("Logged out from Galaxia Cloud")
			}

			this.log("[auth] Logged out from Galaxia Cloud")
		} catch (error) {
			this.log(`[auth] Error logging out from Galaxia Cloud: ${error}`)
			throw new Error(`Failed to log out from Galaxia Cloud: ${error}`)
		}
	}
