...
import { GalaxiaEventName, type ClineMessage } from "@galaxia/types"

import { waitFor, sleep } from "../utils"
import { setDefaultSuiteTimeout } from "../test-utils"

suite.skip("Galaxia use_mcp_tool Tool", function () {
	...
	test("Should request MCP filesystem read_file tool and complete successfully", async function () {
		...
		api.on(GalaxiaEventName.Message, messageHandler)
		...
		api.on(GalaxiaEventName.TaskStarted, taskStartedHandler)
		...
		api.on(GalaxiaEventName.TaskCompleted, taskCompletedHandler)
		await sleep(2000) // Wait for Galaxia to fully initialize
		...
		try {
			...
			// Make a small modification to trigger the save event, without this Galaxia won't load the MCP server
		...
		try {
			...
			// Clean up
			api.off(GalaxiaEventName.Message, messageHandler)
			api.off(GalaxiaEventName.TaskStarted, taskStartedHandler)
			api.off(GalaxiaEventName.TaskCompleted, taskCompletedHandler)
		}
	})
