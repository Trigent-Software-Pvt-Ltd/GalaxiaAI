...
import { type TokenUsage, GalaxiaEventName, taskEventSchema } from "@galaxia/types"
import type { Run, Task, TaskMetrics } from "@galaxia/evals"
...
export const useRunStatus = (run: Run): RunStatus => {
	...
	const onMessage = useCallback((messageEvent: MessageEvent) => {
		...
		switch (eventName) {
			case GalaxiaEventName.TaskStarted:
				startTimes.current.set(taskId, Date.now())
				break
			case GalaxiaEventName.TaskTokenUsageUpdated: {
			...
			case GalaxiaEventName.EvalPass:
			case GalaxiaEventName.EvalFail:
